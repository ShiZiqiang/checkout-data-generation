...
def get_bbox_and_segmentation_of_single_object(mask, image_size):
    ...
    #print('size a {}'.format(size[0]))
    #print('size b {}'.format(size[1]))
    area = size[0]*size[1]
    return segmentation, bbox, area.tolist() # todo: calc area
...
def save_single_annotation_data_to_json(
    ...
    with open(output_file_path, "w") as f:
        ...
        json.dump(subset_dict, f, indent=1)
...
def save_annotation_data_to_json(
    ...
    #    json.dump(subset_dict, f, indent=1)
    #pass
    #json_fp = open(output_file_path, 'w')
    #json_str = json.dumps(subset_dict, indent=1)
